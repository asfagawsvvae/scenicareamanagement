      ...
      <el-row>
        ...
        <el-col :span="12">
          <div class="card-content">
            ...
            <p class="leftParkingSpaces">剩余停车位: {{ item.parkingSpaces - item.occupiedCapacity }}&nbsp辆</p>
          </div>
        </el-col>
      </el-row>
      <el-row>
...
const doAddReservation = async () => {
  ...
  if (reservationModel.value.hotel != "无") {
    ...
    const res = await getHotelNum(params);

    if (res.code == 200) {
      maxRoomsNum.value = res.datas.maxRooms;
      OccupiedRoomsNum.value = res.datas.occupiedRooms;
      HotelParkingNum.value = res.datas.parkingSpaces;
      // console.log(OccupiedRoomsNum.value);
      // console.log(maxRoomsNum.value);
...
const doGetAgencyNameList = () => {
  let params={
    international:getStorage("item_international")
  }


  getAgencyNameList(params).then((res) => {
